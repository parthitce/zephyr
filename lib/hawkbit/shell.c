...
static void cmd_run(const struct shell *shell, size_t argc, 
		   char **argv)
{
	ARG_UNUSED(argc);
	ARG_UNUSED(argv);
	...
	case HAWKBIT_UNCONFIRMED_IMAGE:
		shell_fprintf(shell, SHELL_ERROR, "Image is unconfirmed." 
...
static int cmd_info(const struct shell *shell, size_t argc, char *argv)
{
	...
	hawkbit_get_device_identity(device_id, DEVICE_ID_MAX_SIZE);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_hawkbit, SHELL_CMD(info, NULL, "Dump Hawkbit information",
						      cmd_info),
