...
#if defined CONFIG_LIS2DH_PULSE
 	u8_t reg_val;

	status = lis2dh_reg_read_byte(dev, LIS2DH_REG_CTRL2, &reg_val);
	if (status < 0) {
		LOG_ERR("Unable to read register");
		return status;
	}

        status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CTRL2, reg_val | LIS2DH_HPCLICK_EN_BIT);
        if (status < 0) {
                LOG_ERR("Click high pass filter enable failed (%d)", status);
		return status;
        }

	status = lis2dh_reg_read_byte(dev, LIS2DH_REG_CTRL6, &reg_val);
	if (status < 0) {
		LOG_ERR("Unable to read register");
		return status;
	}

        status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CTRL6, reg_val | LIS2DH_EN_CLICK_INT2);
        if (status < 0) {
                LOG_ERR("Click enable write failed (%d)", status);
		return status;
        }
#if defined CONFIG_LIS2DH_ONLY_SINGLE
        status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_CFG, LIS2DH_SINGLE_CLICK_EN);
        if (status < 0) {
                LOG_ERR("Unable to configure single tap");
		return status;
        }

       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_THS, CONFIG_LIS2DH_CLICK_THS);
       if (status < 0) {
               LOG_ERR("Unable to configure single tap threshold");
       	       return status;
       }

       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_TIME_LMT, CONFIG_LIS2DH_CLICK_TIME_LIMIT);
       if (status < 0) {
                LOG_ERR("Unable to set single tap time limit");
		return status;
       }

       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_TIME_LAT, CONFIG_LIS2DH_CLICK_TIME_LATENCY);
       if (status < 0) {
                LOG_ERR("Unable to set double tap time latency");
		return status;
       }

#elif defined CONFIG_LIS2DH_SINGLE_DOUBLE
	
       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_CFG, LIS2DH_SINGLE_DOUBLE_CLICK_EN);
       if (status < 0) {
               LOG_ERR("Failed to set single and double tap");
		return status;
        }

       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_THS, CONFIG_LIS2DH_CLICK_THS);
       if (status < 0) {
                LOG_ERR("Unable to configure double tap threshold");
		return status;
       }

       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_TIME_LMT, CONFIG_LIS2DH_CLICK_TIME_LIMIT);
       if (status < 0) {
               LOG_ERR("Unable to set double tap time limit");
	       return status;
       }
       ...
       status = lis2dh_reg_write_byte(dev, LIS2DH_REG_CLICK_TIME_WINDOW, CONFIG_LIS2DH_CLICK_TIME_WINDOW);
       if (status < 0) {
               LOG_ERR("Unable to set duble tap time window");
	       return status;
       }
...
int lis2dh_acc_activity_config(struct device *dev, enum sensor_attribute attr,
			    const struct sensor_value *val)
{
	int status;

	if (attr == SENSOR_ATTR_ACTIVITY_TH) {
		...
		LOG_INF("activity_ths=0x%x range_g=%d ums2=%u", reg_val,
			    range_g, slope_th_ums2 - 1);


		status = lis2dh_reg_write_byte(dev, LIS2DH_REG_ACT_THS, reg_val);
		...
		LOG_INF("activity_dur=0x%x", val->val1);

		status = lis2dh_reg_write_byte(dev, LIS2DH_REG_ACT_DUR, val->val1);
	}
...
static void lis2dh_thread_cb(void *arg)
{
	...
	if (atomic_test_and_clear_bit(&lis2dh->trig_flags,
		...
		struct sensor_trigger single_click = {
                        .type = SENSOR_TRIG_TAP,
                };

                struct sensor_trigger double_click = {
                        .type = SENSOR_TRIG_DOUBLE_TAP,
                };

                status = lis2dh_reg_read_byte(dev, LIS2DH_REG_CLICK_SRC,
                                                        &click_reg_val);
...
#if defined CONFIG_LIS2DH_PULSE
                if ((click_reg_val & 0x40) != 0)
                {
			if ((click_reg_val & LIS2DH_SINGLE_CLICK_BIT) != 0) {
                        	 lis2dh->handler_anymotion(dev, &single_click);
                         }
                        if ((click_reg_val & LIS2DH_DOUBLE_CLICK_BIT) !=  0) {
                                        lis2dh->handler_anymotion(dev, &double_click);
                         }
                }
#endif
